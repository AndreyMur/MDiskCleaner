# Инвентаризация функций приложения DiskCleaner

> Составлено по результатам исследования кодовой базы (src/) и документации (docs/).
> Приложение: Windows-утилита безопасной очистки диска. Стек: .NET 8, C# 12, WPF + MVVM (CommunityToolkit.Mvvm), Serilog.
> Состав решений: `DiskCleaner.Gui` (WPF), `DiskCleaner.Cli` (консоль), `DiskCleaner.Core` (ядро), `DiskCleaner.Elevated` (процесс с правами админа), тесты в `tests/`.

Общий цикл работы: **обнаружить → показать с оценкой места → подтвердить → безопасно выполнить (UAC только где нужно) → отчитаться, сколько освобождено**. Ничего не удаляется молча.
